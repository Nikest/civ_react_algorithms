...
export class PlanetMiniterra extends AbstractPlanet {
    constructor({ seed, temperatureZone, orbitRadius, starMass }: IAbstractPlanetProps) {
        ...
        if (temperatureZone === 1) {
            this.atmospherePressure = utils.easeInExpoRounded(0, 5, this.rand.randomFloat(0, 1), 2);
        } else if (temperatureZone === 2) {
            this.atmospherePressure = utils.easeInExpoRounded(0, 10, this.rand.randomFloat(0, 1), 2);
